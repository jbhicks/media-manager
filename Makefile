...
install:
ifeq ($(OS),Windows_NT)
	@powershell -NoProfile -ExecutionPolicy Bypass -File "scripts/stop-media-manager.ps1"
endif
	$(MAKE) build
ifeq ($(OS),Windows_NT)
	...
	@powershell -NoProfile -ExecutionPolicy Bypass -File "scripts/install-windows-launch.ps1"
else
	@install_dir="${GOBIN}"; \
